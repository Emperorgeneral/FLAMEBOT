...
class MainWindow(QtWidgets.QWidget):
    ...
    def _enter_psl_edit_mode(self) -> None:
        ...
        if not getattr(self, "flamebot_id", None):
            ...
            return
        # Account-mismatch toast removed intentionally; editing is tied strictly to the selected account's EA state.
    ...
    def _enter_edit_mode(self) -> None:
        ...
        if not getattr(self, "flamebot_id", None):
            self._show_toast("❌ FlameBot ID missing. Please register/login first.")
            return
        # No mismatch toast; editing simply follows the selected account's EA state.
    ...
    def _update_active_profile_label(self) -> None:
        ...
        try:
            ...
            pass
        # Determine per-account activity
        try:
            prop_state = self._get_ea_context_state(self.platform, "prop")
            normal_state = self._get_ea_context_state(self.platform, "normal")
            prop_on = bool(prop_state.get("ea_active"))
            normal_on = bool(normal_state.get("ea_active"))
        except Exception:
            prop_on = False
            normal_on = False

        if not (prop_on or normal_on):
            self.active_profile_label.setText(f"Platform: {platform_label} | EA not connected")
            return

        prop_mask = self._mask_license_key(self.licenses.get("prop", ""))
        normal_mask = self._mask_license_key(self.licenses.get("normal", ""))

        if prop_on and normal_on:
            self.active_profile_label.setText(
                f"Platform: {platform_label} | Active on EA: PROP • {prop_mask} | NORMAL • {normal_mask}"
            )
            return

        if prop_on:
            self.active_profile_label.setText(
                f"Platform: {platform_label} | Active on EA: PROP • {prop_mask}\nInactive on EA: NORMAL • {normal_mask}"
            )
            return

        # else only normal is on
        self.active_profile_label.setText(
            f"Platform: {platform_label} | Active on EA: NORMAL • {normal_mask}\nInactive on EA: PROP • {prop_mask}"
        )
    ...
    def _apply_platform_ea_state_from_response(self, auth_params: dict, response: Optional[dict]) -> None:
            ...
            Merge conservatively: never flip the other account to False unless the
            response explicitly asserts it. This allows PROP and NORMAL to be
            simultaneously active when using different license keys.
            """
            ...
            # Target account from the auth context (may be None)
            ...
            if ea_active_value is not None:
                ...
                if not ea_active:
                    # Only mark the targeted account (if known) as inactive; do not wipe the other.
    ...
    def _is_settings_unlocked(self, platform: Optional[str] = None) -> bool:
        """Unlocked only when the EA for the selected account is active on this platform.

        PROP and NORMAL are independent; being active on one does not unlock the other.
        """
        platform_key = (platform or getattr(self, "platform", None) or "mt5").lower()
        selected_account = self._normalize_account_type(getattr(self, "current_account_type", None) or "prop")
